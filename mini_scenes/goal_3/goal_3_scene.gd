extends MiniScene
## What: Hand-washing mini-game with water/soap detection
## Who: MiniScene parent, XROrigin hand tracking, Area3D signals
## Why: Goal 3 good health - teach proper handwashing technique
...
const WATER_DETECTION_RADIUS: float = 0.35
const SOAP_CONTACT_RADIUS: float = 0.20
const WATER_FILL_RATE: float = 1.0 / 20.0
const WATER_DRAIN_RATE: float = 1.0 / 40.0
const SOAP_FILL_RATE: float = 1.0 / 10.0
const SOAP_DRAIN_RATE: float = 1.0 / 20.0
...
func _get_references() -> void:
	...
	if has_node("Soap"):
		...
		soap_body.freeze = true  # until scene ready
...
func _setup_water_detection_area() -> void:
	...
	if water_zone_marker:
		...
		water_detection_area.position = Vector3(-0.1, 0.58, -0.26)
...
func _setup_water_particles() -> void:
	...
	if water_spout_marker:
		...
		water_particles.position = Vector3(-0.08, 0.66, -0.22)
...
func _physics_process(delta: float) -> void:
	...
	# Manual gravity: world gravity = 0
...
func _check_hands_in_water() -> void:
	# Primary: Area3D signals (_on_water_zone_area_entered/exited)
	# Fallback: distance check with scene scale adjustment
